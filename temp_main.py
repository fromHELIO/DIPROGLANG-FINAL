            ...
            try: #checks if quantity is a number
                ...
                try: #checks if price is a float
                    ...
                    try: #checks if date is valid
                        ...
                        cursor.execute(f"INSERT INTO change_log(employee_id, change_type) VALUES('{getEmpId()}', 'ADDED INVENTORY {new_item} - {name}')")
...
class DeleteRecord(Screen):
    ...
    def deleteInventory(self):
        ...
        try:
            ...
            popup = Popup(title='Invalid Item ID', content=Label(text='Please input a valid input ID.'), size_hint=(None, None), size=(400, 200))
            popup.open()
        else:
            #insert to changelog
            cursor.execute(f"SELECT name FROM inventory where id = '{itemId}'")
            deleted_Name = cursor.fetchall()[0][0]
            
            cursor.execute(f"INSERT INTO change_log(employee_id, change_type) VALUES('{getEmpId()}', 'DELETED INVENTORY {itemId} - {deleted_Name}')")
